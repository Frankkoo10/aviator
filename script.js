...
// --- NUEVAS VARIABLES DE SINCRONIZACIÓN SECUENCIAL ---
const WAIT_TIME_MS = 20000;    // Tiempo de espera para apostar: 20 segundos
const CRASH_DELAY_MS = 3000;   // Tiempo mostrando la explosión antes de reiniciar (3 seg)

let estadoActual = 'WAITING';  // 'WAITING', 'FLYING', 'CRASHED'
let currentRoundId = parseInt(localStorage.getItem('aviator_current_round')) || 1;

let tiempoInicioEspera = 0;
let tiempoInicioVuelo = 0;
let tiempoInicioCrash = 0;
let puntoChoqueActual = 1.00;
let crashTimeMsActual = 0;
...
// ==========================================
// 5. MOTOR DEL JUEGO (SECUENCIAL Y CONTINUO)
// ==========================================
...
function iniciarMotorJuego() {
    ...
    if (savedBet) {
        if (savedBet.roundId === currentRoundId) {
    ...
    iniciarNuevaRonda();
    requestAnimationFrame(loopJuegoLocal);
}

function iniciarNuevaRonda() {
    puntoChoqueActual = calcularPuntoChoque(currentRoundId);
    crashTimeMsActual = (Math.log(puntoChoqueActual) / 0.25) * 1000;
    
    // Tope visual
    if (crashTimeMsActual > 28000) {
        crashTimeMsActual = 28000;
        puntoChoqueActual = parseFloat(Math.exp(0.25 * 28).toFixed(2));
    }
    if (puntoChoqueActual < 1.00) puntoChoqueActual = 1.00;

    tiempoInicioEspera = Date.now();
    transitionToWaiting();
}
...
function loopJuegoLocal() {
    let nowMs = Date.now();

    if (estadoActual === 'WAITING') {
        let elapsed = nowMs - tiempoInicioEspera;
        let timeLeftMs = WAIT_TIME_MS - elapsed;

        if (timeLeftMs <= 0) {
            tiempoInicioVuelo = nowMs;
    ...
    } else if (estadoActual === 'FLYING') {
        let timeFlyingMs = nowMs - tiempoInicioVuelo;

        if (timeFlyingMs >= crashTimeMsActual) {
            tiempoInicioCrash = nowMs;
            transitionToCrashed(puntoChoqueActual);
        } else {
            let multi = Math.max(1.00, Math.exp(0.25 * (timeFlyingMs / 1000)));
            actualizarVueloUI(multi);
        }
    } else if (estadoActual === 'CRASHED') {
        let elapsed = nowMs - tiempoInicioCrash;
        if (elapsed >= CRASH_DELAY_MS) {
            // Avanzar a la siguiente ronda automáticamente sin esperas de ciclos largos
            currentRoundId++;
            localStorage.setItem('aviator_current_round', currentRoundId);
            iniciarNuevaRonda();
        }
    }
